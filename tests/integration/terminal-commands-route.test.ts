...
import { POST } from "@/app/api/terminal/sessions/[id]/commands/route";

function createRequest(command: string): NextRequest {
  return {
    nextUrl: new URL(
      "http://localhost:3000/api/terminal/sessions/session-1/commands?project=%2Fworkspace%2Fdemo"
    ),
    json: async () => ({ command }),
  } as unknown as NextRequest;
}

describe("POST /api/terminal/sessions/[id]/commands", () => {
  beforeEach(() => {
    vi.clearAllMocks();
